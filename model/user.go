...
// 模型上仍写 gorm:"unique"（业务语义：用户名全局唯一）；
// 真正的复合索引由 model/init.go 的 ensureSoftDeleteUniqueIndexes 迁移建好。
type User struct {
	BaseModel

	Username string `gorm:"unique;not null;size:50;comment:用户名" json:"username"` // 唯一，登录凭证之一（复合索引见 init.go）
	Email    string `gorm:"unique;not null;size:100;comment:邮箱" json:"email"`    // 唯一，登录凭证之一（复合索引见 init.go）
