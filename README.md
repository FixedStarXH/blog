...
- 仪表盘数据统计（含近 7/30 天浏览量趋势图）；文章管理（发布 / 草稿 / 置顶 / 定时 / 软删除 / 回收站）
- 用户管理（角色分配 / 禁用）、分类 / 标签 / 专栏管理
- 评论审核（通过 / 驳回 / 批量）、图片上传图库、站点设置、友情链接
- **操作审计日志**：后台写操作与登录尝试全量记录（谁/何时/做了什么/成败/IP），支持动作筛选分页，仅管理员可清空——防篡改、可追溯
...
- **操作审计**：admin 组写操作中间件零侵入全量记录（含登录尝试），目标类型/ID/详情/IP 可追溯
- **工程化**：pprof 性能分析（仅管理员可访问）、优雅退出（SIGTERM 触发，等待在途请求处理完成）
