...
- **特色**：浏览量 IP 防刷、文章点赞（IP/用户去重）、私密文章（密码解锁）
...
```
Controller(HTTP)  →  Service(业务)  →  Repository(数据)  →  GORM → MySQL
                                              ↘
                                            Model(实体 + 行为)
```

- **分层架构**：Controller / Service / Repository / Model 四层，依赖方向自上而下单向
- **依赖注入**：构造函数注入，Repository 接口抽象，便于 mock 测试
- **JWT + RBAC**：登录签发 Token，中间件按 `role >= N` 校验权限
...
├── repository/          数据层（接口 + GORM 实现）
├── service/             业务层
├── controller/          HTTP 层
├── middleware/          中间件（鉴权 / 角色 / 日志）
...
> 现状：`dao/` 目录已按此规划重构为 `repository/`（见开发流程阶段二）。
...
| 文档 | 说明 |
|------|------|
| [docs/需求文档.md](docs/需求文档.md) | 功能全景需求（多用户 CMS） |
| [docs/接口设计.md](docs/接口设计.md) | 接口契约（公共 + 用户 + 投稿 + 后台） |
| [docs/数据库设计.md](docs/数据库设计.md) | 12 张表结构设计 |
| [docs/项目设计规范.md](docs/项目设计规范.md) | 工程规范（分层 / 命名 / 安全 / Git） |
| [docs/开发流程.md](docs/开发流程.md) | 7 阶段开发路线（含 curl 验证） |
| [docs/项目审核报告.md](docs/项目审核报告.md) | 简历级质量评估 |
...
- 核心依赖：Gin（Web）、GORM（ORM）、Viper（配置）、golang-jwt（JWT）、x/crypto（BCrypt）

## 🛡️ 安全设计

- 密码 BCrypt 哈希存储
- JWT + RBAC 鉴权（后台接口 `role>=2`，用户管理 `role=3`）
- SQL 全参数化（GORM），杜绝注入
